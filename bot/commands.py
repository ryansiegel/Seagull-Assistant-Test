import discord,asyncio,os,re,speedtest,random,requests,string,ijson,os.path
...
async def rankList(inter, channelPrint, meta, ranks):
    ...
    if meta == "Great":
        meta = "Great League"
        parser = ijson.parse(open(os.path.abspath("all-rankings-1500.json"))
    embedVar = discord.Embed(title="Bidoof Facts!", description="", color=0x000000)
    for prefix, event, value in parser:
        if count <= 10:
            if prefix.endswith('.speciesName'):
                ...
                if event == 'end_map':
                    embedVar.addfield(name="#" + str(count) + ": " + pokemon,value=".", inline=False)
                    matchoppt.clear()
                    matchrate.clear()
                    countoppt.clear()
                    countrate.clear()
